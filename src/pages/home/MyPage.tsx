...
export default function MyPage() {
  ...
  const canSave = (() => {
    ...
    const prev = (profile?.nickname ?? '').trim();
      ...
      <div className="text-green1 mt-6 flex flex-col gap-6 px-8">
        <div>
          ...
          <input
            ...
            value={email}
            readOnly
            className="rounded-m border-gray2 text-body1 bg-gray1 w-full border px-3 py-2"
          />
        </div>

        <div>
          <label className="text-title4 text-green1 mb-1 block">닉네임</label>
          <div className="flex items-center gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && canSave) handleSaveNickname();
              }}
              className="rounded-m border-gray1 text-body1 h-10 w-full border bg-white px-3"
              placeholder="닉네임 입력"
            />
            <button
              type="button"
